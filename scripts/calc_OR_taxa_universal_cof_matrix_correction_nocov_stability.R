...
get_stat=function(testi,refi,tmpi) {
	...
	rm(out)
	tryCatch(
		expr={out=epi.2by2(ss)$massoc.detail},
		error = function(e){print(paste("Error: epi.2by2",colnames(tmpi)[1]))}
	)
	if (!exists("out")) {
		ss=ss+0.5
	}
	...
	colnames(outf)=c("Test","Reference","OR.strata.wald","OR.strata.wald.CI2.5",
		"OR.strata.wald.CI97.5",
